...
#connection à notre cluster ES
es = Elasticsearch(cluster='test_app')

#liste des champs a recuperer avec la requete
lFields = ['text', 'id_str', 'timestamp_ms']

print(es.count(index = 'twitter2015')['count'], 'document in index')

rs = es.search(index='twitter2015', doc_type='tweet', size='1',
               sort=['timestamp_ms:asc'], _source='timestamp_ms', stored_fields='timestamp_ms',
               body={'query': {'match_all': {}}})

for hit in rs['hits']['hits']:
    print(hit['_source']['timestamp_ms'])

rs = es.search(index = 'twitter2015', doc_type = 'tweet', scroll = '10s', size = 1000, sort = ['timestamp_ms:asc'],
               _source = lFields, stored_fields = lFields, body = {'query': {'match_all': {}}})

rs = es.search(index='twitter2015', doc_type='tweet', size='1000', scroll = '10s',
               sort=['timestamp_ms:asc'], _source='timestamp_ms', stored_fields='timestamp_ms',
               body={'query': {'range': {'timestamp_ms': {'gte': '1449493211046'}}}})



timeBegin = '1449493211046'
tmpInt = int('1449493211046') + (2 * 60000)
timeEnd = str(tmpInt)

print('timeBegin -> ' + timeBegin)
print('tmpInt    -> ' + str(tmpInt))
print('timeEnd   -> ' + timeEnd)

rs = es.search(index='twitter2015', doc_type='tweet', size='300', scroll='10s',
               sort=['timestamp_ms:asc'], _source=lFields, stored_fields=lFields,
               body={'query': {'range': {'timestamp_ms': {'gte': timeBegin}}}})
...
while (i < 100):
    try:
        rs2 = es.scroll(scroll_id = sid, scroll = '10s')
        print(i)
